...
class RulesService:
    ...
    async def get_rules_text(cls, bot: discord.Client) -> Tuple[str, str, str]:
        ...
        if source_type == "none":
            ...
            text, path = await cls._load_from_file(rules_file_path)
            if text:
            ...
            text, details = await cls._load_from_channel(bot, rules_channel_id)
            if text:
            ...
            if text:
                ...
                # Prioridad 2-4: Canal de Discord (Adjunto > Fijados > Curados)
    ...
    @classmethod
    async def _load_from_file(cls, path: str) -> Tuple[str, str]:
    ...
    @classmethod
    async def _load_from_channel(cls, bot: discord.Client, channel_id: int) -> Tuple[str, Dict[str, Any]]:
        """
        ...
        3. Mensajes curados recientes (últimos 20 seleccionados por scoring).
        """
        if not channel_id:
            return "", {}
        if not bot.is_ready():
        ...
        if not permissions.read_messages or not permissions.read_message_history:
            logger.error(f"El bot carece de permisos de lectura en el canal: {channel.name} (ID: {channel_id})")
            return "", {}

        # Estrategia A: Buscar archivo adjunto .txt o .md más reciente
        try:
        ...
        # Estrategia C: Últimos mensajes curados del canal (scoring por staff y formato)
        try:
            ...
            if messages_to_consider:
                # Ordenar por score descendente, luego tomar los 20 mejores
