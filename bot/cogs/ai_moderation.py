...
class AISuggestionModal(discord.ui.Modal, title="Asistente de Sanciones IA"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        # 1. Comprobación rápida de Toggle de IA
        if not config.AI_ENABLED:
            await interaction.response.send_message(
                "⚠️ **Asistencia de IA Desactivada:** No se pudo procesar la llamada de la IA. Por favor, configura una `AI_API_KEY` válida en tu archivo `.env`.",
                ephemeral=True
            )
            return
        ...
        # 2. Consultar recomendación de sanción a la IA
        ...
        # 3. Ofrecer botón para registrar la nota administrativamente en base de datos
        class CommitNoteView(discord.ui.View):
            ...
            async def btn_commit(self, btn_interaction: discord.Interaction, button: discord.ui.Button):
                ...
                # Intentar parsear una acción sugerida corta a partir del texto largo de la IA
                ...
                # Guardar nota administrativamente respetando la privacidad
                ...
                # Desactivar botón tras guardado
...
class AIModerationCog(commands.Cog):
    ...
    async def ai_review_user(self, interaction: discord.Interaction, member: discord.Member):
        """
        Slash command que analiza el perfil del miembro mediante IA.
        """
        # 1. Comprobación rápida de Toggle de IA
        if not config.AI_ENABLED:
            await interaction.response.send_message(
                "⚠️ **Asistencia de IA Desactivada:** No se pudo procesar la solicitud. Un administrador debe configurar la `AI_API_KEY` en el archivo `.env` para habilitar el motor de análisis.",
                ephemeral=True
            )
            return
        ...
        # 3. Invocar al servicio cognitivo asíncrono
    ...
    async def ai_suggest_action(self, interaction: discord.Interaction, member: discord.Member):
        """
        Slash command que abre un modal interactivo de Discord para ingresar un incidente.
        """
        # 1. Comprobación rápida de Toggle de IA
        if not config.AI_ENABLED:
            await interaction.response.send_message(
                "⚠️ **Asistencia de IA Desactivada:** El asistente conductual requiere una `AI_API_KEY` activa en tu archivo `.env`.",
                ephemeral=True
            )
            return

        # 2. Abrir Modal de Discord
